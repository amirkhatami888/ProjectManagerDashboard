...
def project_add_allocation(request, pk):
    ...
    if request.user.is_admin:
        ...
        # Province managers can add allocation to projects from their assigned provinces
        user_provinces = request.user.get_assigned_provinces()
        if user_provinces and project.province in user_provinces:
            # User can add allocation to projects from their assigned provinces
            pass
        elif project.created_by == request.user:
            # User can add allocation to their own projects (fallback)
            pass
        else:
