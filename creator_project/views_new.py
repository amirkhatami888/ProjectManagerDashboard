...
def project_reject(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.warning(request, "پروژه رد شد و به حالت پیش‌نویس برگشت داده شد.")
            return redirect('creator_project:project_detail', pk=project.pk)
    else:
        form = ProjectRejectionForm()
    
    return render(request, 'creator_project/project_reject.html', {
        'project': project,
        'form': form
    })
...
def project_financials(request, pk):
    ...
    # Check permissions
    if not (request.user.is_admin or request.user.is_ceo or request.user.is_chief_executive or 
            request.user.is_vice_chief_executive or project.created_by == request.user):
...
def project_add_allocation(request, pk):
    ...
    # Only admins, experts or the project creator can add allocations
    if not (request.user.is_admin or request.user.is_expert or project.created_by == request.user):
        return HttpResponseForbidden("You don't have permission to add allocations to this project.")
    
    if request.method == 'POST':
        form = ProjectFinancialAllocationForm(request.POST)
        
        # The form data has already been converted to Gregorian format by JavaScript
        # so we can use it directly without further conversion here
        ...
        if form.is_valid():
            ...
            messages.success(request, "تخصیص مالی با موفقیت ثبت شد.")
            return redirect('creator_project:project_detail', pk=project.pk)
    else:
