...
:: NSSM shows usage when run without parameters (exit code 1), which is normal
"%NSSM_DIR%\win64\nssm.exe" >nul 2>&1
if %errorLevel% equ 1 (
...
:: Check if Python is installed - try multiple common paths
set PYTHON_FOUND=0
...
echo Services installed successfully with NSSM!
echo.
echo NSSM Service Configuration:
echo - nginx: Auto-start, auto-restart on failure, 5s restart delay
echo - DjangoProjectManager: Auto-start, auto-restart on failure, 10s restart delay
echo - Both services configured with proper logging and graceful shutdown
