...
echo ========================================
echo Django Website Deployment with win-acme
echo Auto-configure SSL with win-acme
echo ========================================
...
set WINACME_DIR=%PROJECT_DIR%win-acme
...
:: Step 2: Verify win-acme installation
echo.
echo [2/12] Verifying win-acme installation...
...
:: Step 8: Configure nginx for HTTP (before SSL)
echo.
echo [8/12] Configuring nginx for HTTP...
...
:: Create initial nginx configuration (HTTP only, win-acme will add HTTPS)
(
echo #user  nobody;
...
echo.
echo [9/12] Installing Windows services...
...
echo - win-acme: %WINACME_DIR%\logs\
echo.
echo SSL Management:
echo - Manual renewal: setup_ssl_winacme.bat
echo - Auto-renewal: Configured (runs weekly on Sundays at 2 AM)
echo ========================================
